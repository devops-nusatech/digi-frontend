...
describe('Api Keys selectors', () => {
   ...
   it('should select selectApiKeysDataLoaded', () => {
      expect(selectApiKeysDataLoaded(state)).toEqual(
   ...
   it('should select selectApiKeysPageIndex', () => {
      expect(selectApiKeysPageIndex(state)).toEqual(
         state.user.apiKeys.pageIndex
      );
   });

   it('should select selectApiKeysFirstElemIndex', () => {
      expect(selectApiKeysFirstElemIndex(state, 4)).toEqual(
         state.user.apiKeys.pageIndex * 4 + 1
      );
   });
   ...
   it('should select selectApiKeysNextPageExists', () => {
      expect(selectApiKeysNextPageExists(state)).toEqual(

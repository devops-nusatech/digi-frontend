...
export const selectApiKeysLoading = (
   state: RootState
): ApiKeysState['loading'] => state.user.apiKeys.loading;

export const selectApiKeysDataLoaded = (
   state: RootState
): ApiKeysState['dataLoaded'] => state.user.apiKeys.dataLoaded;

export const selectApiKeysModal = (
   state: RootState
): ApiKeys2FAModal['payload'] => state.user.apiKeys.modal;

export const selectApiKeysPageIndex = (state: RootState): number =>
   state.user.apiKeys.pageIndex;

export const selectApiKeysFirstElemIndex = (
   state: RootState,
   limit: number
): number => state.user.apiKeys.pageIndex * limit + 1;

export const selectApiKeysLastElemIndex = (
   state: RootState,
   limit: number
): number =>

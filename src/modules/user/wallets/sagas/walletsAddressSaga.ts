...
export function* walletsAddressSaga(action: WalletsAddressFetch) {
   try {
      ...
      const blockchainKey = action.payload.blockchainKey?.toLocaleLowerCase();
      const url = `/account/deposit_address/${currency}?blockchain_key=${blockchainKey}`;
      const { address, currencies, state, blockchain_key } = yield call(
         API.get(walletsAddressOptions),
         url
      );
      yield put(
         walletsAddressData({ address, currencies, state, blockchain_key })
      );
   } catch (error) {
      yield put(
         sendError({

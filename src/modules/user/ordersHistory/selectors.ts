...
export const selectOrdersFirstElemIndex = (
   state: RootState,
   limit: number
): number => state.user.ordersHistory.pageIndex * limit + 1;

export const selectOrdersLastElemIndex = (
   state: RootState,
   limit: number
): number =>
   state.user.ordersHistory.pageIndex * limit +
   state.user.ordersHistory.list.length;

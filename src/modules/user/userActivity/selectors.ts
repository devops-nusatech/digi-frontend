...
export const selectUserActivityFirstElemIndex = (
   state: RootState,
   limit
): number => state.user.userActivity.page * limit + 1;

export const selectUserActivityLastElemIndex = (
   state: RootState,
   limit: number
): number => {
   if (
...
export const selectUserActivityNextPageExists = (
   state: RootState,
   limit: number
): boolean =>

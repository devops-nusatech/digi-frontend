...
const getLayouts = () => {
   ...
   const gridHeight = Math.max(
      minGridHeight,
      window.innerHeight - staticHeight - margin * 3
   );

   const currentTradingChartHeight = pixelsToGridUnits(
      gridHeight -
         margin * 2 -
         gridUnitsToPixels(orderHeight, rowHeight, margin),
      rowHeight,
      margin
   );
   const tradingChartHeight =
      gridHeight >= minGridHeight
         ...
         : Math.floor(
              Math.max(currentTradingChartHeight, minTradingChartHeight)
           );

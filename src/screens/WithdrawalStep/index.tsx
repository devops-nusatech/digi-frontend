...
const WithdrawalFC = memo(
   ({
      ...
      const [{ address, label, description, destinationTag }, setField] =
         useForm<State>({
            accountName: '',
            accountNumber: '',
            bank: '',
      ...
      useEffect(() => {
         if (location.state?.wallet) {
            setStepActive(2);
            setWalletActive(
               location.state?.wallet && location.state?.wallet?.currency
            );
            window.scrollTo({
               ...
               <div className="hidden space-x-4">
                  {userWallets[0]?.networks.map((network, index) => (
                     <Button
                        ...
                        fontDM={false}
                        variant={
                           networkActive === index ? 'primary' : 'outline'
                        }
                           ...
                           beneficiaries.map(
                              (
                                 ...
                                 >
                                    ...
                                    <td className="rounded-r-xl p-4 text-right align-middle font-medium transition-all duration-300 group-hover:bg-neutral7 dark:group-hover:bg-neutral2">
                                       <svg className="h-6 w-6 fill-primary4 transition-colors duration-300">
      ...
      const validateCoinAddressFormat = (value: string) => {
         if (
            getCurrencies().some(
               currency => currency.symbol === userWallets[0]?.currency
            )
         ) {
                  ...
                  <>
                     <Listbox
                        ...
                        <div className="relative">
                           ...
                           <Listbox.Options
                              className={({ open }) =>
                                 `absolute z-[9] mt-0.5 max-h-40 w-full overflow-auto rounded-xl border-2 border-neutral6 bg-neutral8 shadow-dropdown-2 outline-none dark:border-neutral1 dark:bg-neutral1 dark:shadow-dropdown-3 ${
                                    open
      ...
      const { currentStep, step, nextStep, prevStep, goToStep } = useStep([
         renderAssetStep,
         <div>Step Kalih</div>,
         <div>Step Tigo</div>,
      ]);
               ...
               <div className="mt-10 space-y-8">
                  ...
                  <div className="flex flex-wrap rounded-xl border border-neutral6 p-6">
                     <div className="mr-auto space-y-2.5">
                        ...
                        <div className="font-medium text-primary5">
                           Completed
                        </div>
                     </div>
                     <div className="space-y-2.5">

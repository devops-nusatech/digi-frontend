...
const getColorSettings = (colorTheme?: string) => {
   ...
   return {
      strokeAreaAskColor: convertRgbToHex(
         getStylesValueByKey(colors.dark.depth.strokeAreaAsk)
      ),
      strokeAreaBidColor: convertRgbToHex(
         getStylesValueByKey(colors.dark.depth.strokeAreaBid)
      ),
      strokeAxisColor: convertRgbToHex(
         getStylesValueByKey(colors.dark.depth.strokeAxis)
      ),
      strokeGridColor: convertRgbToHex(
         getStylesValueByKey(colors.dark.depth.strokeGrid)
      ),
      fillAreaAskColor: convertRgbToHex(
         getStylesValueByKey(colors.dark.depth.fillAreaAsk)
      ),
      fillAreaBidColor: convertRgbToHex(
         getStylesValueByKey(colors.dark.depth.fillAreaBid)
      ),
...
const CustomTooltip = (props: CustomToolTipProps) => {
   ...
   if (active) {
      ...
      const currData = payloadData
         ? external.find(
              (entry: KeyValuePairMarketDepths) =>
                 entry.name === payloadData.name
           )
         : null;

      return (
         <div
            className="area-chart-tooltip rounded-xl"
            style={style}>

...
describe.skip('SignIn component', () => {
   ...
   it('should render logo block', () => {
      ...
      const firstState = wrapper
         .find('.cr-sign-in-form__form-content')
         .children();
   ...
   it('should have correct labels', () => {
      ...
      expect(wrapper.find('.cr-sign-in-form__tab-signup').text()).toBe(
         'label sign up'
      );
   });
   ...
   it('should render error blocks', () => {
      ...
      expect(wrapper.find('.cr-sign-in-form__error').first().text()).toBe(
         'error email'
      );
      expect(wrapper.find('.cr-sign-in-form__error').last().text()).toBe(
         'error password'
      );
   });
   ...
   it('should have correct labels for input fields', () => {
      let wrapper = setup();
      expect(
         wrapper.find('.cr-sign-in-form__bottom-section-password').text()
   ...
   it('should render captcha block', () => {
      let wrapper = setup({
         ...
         renderCaptcha: (
            <div className="cr-sign-in-form__recaptcha">Content</div>
         ),
      });
      captchaLogin()
         ? expect(wrapper.find('.cr-sign-in-form__recaptcha').exists()).toBe(
              true
           )
         : expect(wrapper.find('.cr-sign-in-form__recaptcha').exists()).toBe(
              false

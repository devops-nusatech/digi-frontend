...
export class Markets extends React.Component<MarketsProps, MarketsState> {
   ...
   public componentWillReceiveProps(nextProps: MarketsProps) {
      if (nextProps.data !== this.props.data) {
         this.setState({
            filteredData: nextProps.data.filter(w =>
               (w[0] as string)
                  .toLowerCase()
   ...
   public render() {
      ...
      let tableData = filteredData.length > 0 ? filteredData : [['', '', '']];
   ...
   private mapRows = (cell: CellData) => {
      const isChangeValue =
         typeof cell === 'string' &&
         (cell.charAt(0) === '-' || cell.charAt(0) === '+');

      return isChangeValue ? this.renderChange(cell as string) : cell;
   };

   private filterType =
      (headerKey: string, searchKey: string) => (item: CellData[]) => {
         const typeIndex = this.props.headers
            ? this.props.headers.indexOf(headerKey)
            : this.defaultHeaders.indexOf(headerKey);
   ...
   private getMarketFromDataRow = (market: React.ReactNode[]) =>
      market[0] as string;

   private createUniqueCurrencies(currencies: string[], market: string) {
      const isCurrencyUnique = (currency: string) =>

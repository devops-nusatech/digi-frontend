...
describe('OpenOrders', () => {
   ...
   it('should render formatted price', () => {
      ...
      const renderedBuyPrice = (
         <span className="cr-open-orders__price cr-open-orders__price--buy">
            {sideBuy}
         </span>
      );

      const renderedSellPrice = (
         <span className="cr-open-orders__price cr-open-orders__price--sell">
            {sideSell}
         </span>
      );

      expect(instance.renderAction(sideBuy, sideBuy)).toEqual(renderedBuyPrice);
      expect(instance.renderAction(sideSell, sideSell)).toEqual(
         renderedSellPrice
      );
   });
   ...
   it('should map rows', () => {
      ...
      expect(JSON.stringify(instance.renderRow(row, rowIndex))).toBe(
         JSON.stringify(renderedRow)
      );
   });
});

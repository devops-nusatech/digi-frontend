...
export class OpenOrders extends React.Component<OpenOrdersProps> {
   ...
   public render() {
      ...
      const orderIndex = headersKeys.findIndex(
         header => header === 'Order Type'
      );

      if (headersKeys[orderIndex] === 'Order Type') {
         headers[orderIndex] = (
            <span onClick={this.props.function}>Order Type</span>
         );
      }
   ...
   public renderCell =
      (rowIndex: number) =>
      (cell: CellData, index: number, row: CellData[]) => {
         const { headersKeys = this.defaultHeadersKeys } = this.props;
         const actionIndex = headersKeys.findIndex(
         ...
         switch (index) {
            case actionIndex:
               return this.renderAction(
                  row[actionIndex] as string,
                  row[buySellIndex] as string
               );
            case orderIndex:

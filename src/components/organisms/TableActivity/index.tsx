...
export const TableActivity: FC<TableActivityProps> = ({
   ...
   const fetchUpdate = ({ page }: { page: number }) =>
      fetchHistory({
         page,
         core,
         limit: 10,
      ...
      switch (core) {
         ...
         case 'deposits': {
            ...
            const { amount, created_at, currency, txid, blockchain_key } =
               deposits;
            const blockchainLink = getBlockchainLink(currency, blockchain_key, {
               txid: txid,
            });
            const wallet = wallets.find(obj => obj.currency === currency);
         ...
         case 'withdraws': {
            ...
            const blockchainTXID = getBlockchainLink(currency, blockchain_key, {
                     ...
                     <div className="flex items-center">
                        ...
                        {blockchain_txid && (
                           <AdibTooltip
                              ...
                              <div
                                 className="cursor-copy"
                                 onClick={() => handleCopy(blockchain_txid)}>
                                 <svg className="ml-2 h-4 w-4 fill-neutral4 transition-all duration-300 hover:fill-neutral2">
                                    <use xlinkHref="#icon-copy" />
                                 </svg>
                              </div>

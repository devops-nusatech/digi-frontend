...
export const TradingOrderAsk: FC<TradingOrderAsksProps> = ({
   ...
   useEffect(() => {
      setOrderVolume(
         Decimal.format(availableBase * slide, amountPrecision, ',')
      );
      setOrderTotal(
         Decimal.format(
            convertPrice() * (availableBase * slide),
            pricePrecision,
            ','
         )
      );
   ...
   const isDisabled = (): boolean => {
      ...
      return (
         ...
         invalidAmount ||
         invalidLimitPrice ||
         +Number(orderVolume) > +availableBase ||
         +availableBase < +minAmount
      );
   };

   const totalPrice = () =>
      getTotalPrice(orderVolume, Number(priceMarket), bids);
   const safePrice = () =>

...
class BeneficiariesComponent extends React.Component<Props, State> {
   ...
   public componentWillReceiveProps(nextProps: Props) {
      ...
      if (
         ...
         (nextProps.beneficiaries.length &&
            nextProps.beneficiaries !== beneficiaries)
      ) {
      ...
      if (
         nextProps.beneficiariesActivateSuccess &&
         !beneficiariesActivateSuccess
      ) {
         this.handleToggleConfirmationModal();
      }
   ...
   public render() {
      ...
      const filtredBeneficiaries = this.handleFilterByState(beneficiaries, [
         'active',
         'pending',
      ]);
         ...
         <div className="space-y-3">
            <div className="text-xs font-bold uppercase leading-none text-neutral5">
               {type === 'coin'
                  ...
                  : this.translate(
                       'page.body.wallets.beneficiaries.fiat.title'
                    )}
            </div>
            {filtredBeneficiaries.length
               ? this.renderAddressDropdown(
                    filtredBeneficiaries,
                    currentWithdrawalBeneficiary,
                    type
                 )
   ...
   private renderAddAddress = () => {
      return (
         <div
            ...
            <svg className="h-5 w-5 fill-neutral4 transition-all duration-300 group-hover:fill-neutral2">
   ...
   private renderDropdownItem = (
      ...
      if (type === 'fiat') {
         return (
            <div
               key={index}
               className="group relative cursor-default select-none py-2 pl-10 pr-3.5">
               <div
                  className="item__left"
                  onClick={this.handleClickSelectAddress(item)}>
                  <span className="item__left__title">
                     {this.translate(
                        'page.body.wallets.beneficiaries.dropdown.fiat.name'
                     )}
                  </span>
               ...
               <div
                  ...
                  <span className="item__left__title">
                     {this.translate(
                        'page.body.wallets.beneficiaries.dropdown.fiat.fullName'
                     )}
                  </span>
               ...
               <div className="item__right">
                  {isPending ? (
                     <span className="item__right__pending">
                        {this.translate(
                           'page.body.wallets.beneficiaries.dropdown.pending'
                        )}
                     </span>
                  ) : null}
                  <span
      ...
      return (
         <div
            key={index}
            onClick={this.handleClickSelectAddress(item)}
            className="group relative flex cursor-default select-none items-center justify-between py-2 pl-10 pr-3.5">
            <div>{item?.name}</div>
            <div className="flex items-center space-x-3">
               {isPending && (
                  <div className="text-x text-primary4">
                     {this.translate(
                        'page.body.wallets.beneficiaries.dropdown.pending'
                     )}
                  </div>
               ...
               <TrashBin
                  onClick={this.handleClickDeleteAddress(item)}
                  className="h-4 w-4 fill-neutral2 transition-all duration-300 hover:fill-neutral2"
               />
            </div>
         </div>
      );
   ...
   private renderDropdownBody = (
      ...
      const dropdownBodyClassName = classnames(
         'z-10 absolute mt-1 max-h-60 w-full overflow-auto rounded-xl bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none',
         {
            'fiat-body': type === 'fiat',
         }
      );
         ...
         <div className={dropdownBodyClassName}>
            {beneficiaries &&
               beneficiaries.map((item, index) =>
                  this.renderDropdownItem(item, index, type)
               )}
         </div>
      );
   };
   ...
   private renderDropdownTipCryptoNote = (note: string) => {
      return (
         <div>
            <div className="text-xs font-medium text-neutral4">
               {this.translate(
                  'page.body.wallets.beneficiaries.tipDescription'
               )}
            </div>
            <div>{note}</div>
         </div>
   ...
   private renderDropdownTipCrypto = (
      currentWithdrawalBeneficiary: Beneficiary
   ) => {
      if (currentWithdrawalBeneficiary) {
         return (
            <div className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-xl bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
               <div className="space-y-3 py-2 px-3.5">
                  <div>
                     <div className="text-xs font-medium text-neutral4">
                        Addresss
                     </div>
      ...
      return (
         <div className="tip__content__block">
            <span className="tip__content__block__label">
               {this.translate(
                  'page.body.wallets.beneficiaries.dropdown.fiat.description'
               )}
            </span>
         ...
         return (
            <div className="pg-beneficiaries__dropdown__tip tip fiat-tip">
               <div className="tip__content">
                  ...
                  {currentWithdrawalBeneficiary.description &&
                     this.renderDropdownTipFiatDescription(
                        currentWithdrawalBeneficiary.description
                     )}
                  <div className="tip__content__block">
                     <span className="tip__content__block__label">
                        {this.translate(
                           'page.body.wallets.beneficiaries.dropdown.fiat.account'
                        )}
                     </span>
                     <span className="tip__content__block__value">
                        {
   ...
   private renderAddressDropdown = (
      ...
      if (type === 'fiat') {
         return (
            <div className="relative h-12 w-full rounded-xl border-2 border-neutral6 bg-transparent bg-none pl-3.5 pr-12 font-medium leading-12 shadow-none outline-none transition-all duration-300 focus:border-neutral4 dark:border-neutral3 dark:focus:border-neutral4">
               <div
                  ...
                  <div className="select__right">
                     ...
                     <span className="select__right__select">
                        {this.translate(
                           'page.body.wallets.beneficiaries.dropdown.select'
                        )}
                     </span>
               ...
               {isOpenTip &&
                  this.renderDropdownTipFiat(currentWithdrawalBeneficiary)}
            </div>
         );
      }
         ...
         <div>
            <div className="relative">
               <button
                  type="button"
                  onClick={this.handleToggleDropdown}
                  className="h-12 w-full truncate rounded-xl border-2 border-neutral6 bg-transparent bg-none pl-3.5 pr-17 text-left font-medium leading-12 shadow-none outline-none transition-all duration-300 read-only:bg-neutral7 focus:border-neutral4 dark:border-neutral3 dark:focus:border-neutral4">
                  {currentWithdrawalBeneficiary.data.address}
               </button>
               <div className="absolute right-0 top-0 flex h-12 items-center">
                  <TipIcon
                     onMouseOut={this.handleToggleTip}
                     onMouseOver={this.handleToggleTip}
                     className="h-4 w-4 fill-primary4 stroke-primary4 transition-all duration-300"
                  />
                  <button
                     type="button"
                     onClick={this.handleClickToggleAddAddressModal()}
                     className="flex w-12 items-center justify-center text-center transition-all duration-300">
                     <svg className="h-6 w-6 fill-neutral4 transition-all duration-300 group-hover:fill-neutral2">
                        <use xlinkHref="#icon-plus" />
                     </svg>
                  </button>
               </div>
               {isOpenDropdown && this.renderDropdownBody(beneficiaries, type)}
               {isOpenTip &&
                  this.renderDropdownTipCrypto(currentWithdrawalBeneficiary)}
            </div>
         </div>
      );
   };
...
const mapDispatchToProps: MapDispatchToProps<DispatchProps, {}> = dispatch => ({
   ...
   beneficiariesCreateData: payload =>
      dispatch(beneficiariesCreateData(payload)),

...
export const FormRegisterV2: FC<FormRegisterV2Props> = memo(({
   captchaType,
   renderCaptcha,
   captcha_response,
   isReCaptchaSuccess,
   ...
   const isDisabledButton = useMemo((): boolean => {
      if ((isUsernameEnabled() && !username.match(USERNAME_REGEX)) || !email.match(EMAIL_REGEX) || !password || !confirmPassword || !hasConfirmed || isLoading) return true;
      if (captchaType === 'recaptcha' && !isReCaptchaSuccess) return true;
      if (captchaType === 'geetest' && !isGeetestCaptchaSuccess) return true;
      return false;
   }, [username, email, password, confirmPassword, hasConfirmed, isLoading, captchaType, isReCaptchaSuccess, isGeetestCaptchaSuccess]);
   ...
   return (
      <>
         <form>
            <div className="space-y-7.5" ref={ref} onKeyPress={handleEnterPress}>
               {isUsernameEnabled() && (
                  <InputGroup
                     id="username"
               ...
               <InputGroup
                  ...
                  withIconPassword
               />
               <div className={`relative ${!showFieldRefid && 'h-4 overflow-hidden'} transition-all duration-700`}>
                  <div className="absolute right-0 top-0 z-2">
                     <svg
                        onClick={() => setShowFieldRefid(!showFieldRefid)}
                        className={`cursor-pointer w-6 h-6 fill-neutral4 transition-transform duration-500 ${showFieldRefid && 'rotate-180'}`}
                     >

...
export class CustomizationThemes extends React.Component<Props, State> {
         ...
         <div className="pg-customization-themes__themes">
            ...
            <DropdownComponent
               ...
               placeholder={translate(
                  AVAILABLE_COLOR_THEMES[currentThemeIndex].title
               )}
            />
         </div>
      );
   ...
   public renderColors() {
      return (
         <div className="pg-customization-themes__colors">
            <PerfectScrollbar>
               {AVAILABLE_COLORS_TITLES.map((item, index) =>
                  this.renderColorsItem(item, index)
               )}
            </PerfectScrollbar>
         </div>
      );
   ...
   private handleChangeCurrentTheme = (index: number) => {
      ...
      if (rootElement) {
         AVAILABLE_COLORS_TITLES.reduce((result, item) => {
            const newItemColor = AVAILABLE_COLOR_THEMES[index].theme.find(
               theme => theme.key === item.key
            );
   ...
   private handleApplyCustomizationSettings = (
      customization: CustomizationDataInterface
   ) => {
      const parsedSettings = customization.settings
         ? JSON.parse(customization.settings)
         : null;

      if (parsedSettings && parsedSettings.theme_id) {
         const themeIndexToSet = AVAILABLE_COLOR_THEMES.findIndex(
            theme => theme.id === +parsedSettings.theme_id
         );

         if (themeIndexToSet >= 0) {
            this.handleSetCurrentTheme(themeIndexToSet);
            this.props.handleSetCurrentCustomization(
               'theme_id',
               +parsedSettings.theme_id
            );
         }
      }
   };

   private handleResetCustomizationSettings = (
      customization: CustomizationDataInterface
   ) => {
      const { handleTriggerChartRebuild } = this.props;
      const rootElement = document.documentElement;

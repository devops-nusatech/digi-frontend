...
const OrdersComponent: React.FC = () => {
   ...
   const firstElemIndex = useSelector((state: RootState) =>
      selectOrdersFirstElemIndex(state, 25)
   );
   const lastElemIndex = useSelector((state: RootState) =>
      selectOrdersLastElemIndex(state, 25)
   );
   const ordersNextPageExists = useSelector(selectOrdersNextPageExists);
   const filteredOrders =
      currentTabIndex === 0
         ? orders.filter(o => ['wait', 'pending'].includes(o.state))
         : orders;
   useUserOrdersHistoryFetch(
      currentPageIndex,
      userOrdersHistoryTabs[currentTabIndex],
      25
   );
   ...
   const renderOptionalHead = () => (
      <div
         ...
         <span>
            {intl.formatMessage({ id: 'page.mobile.orders.cancelAll' })}
         </span>
         <CloseIcon />
      </div>
   );

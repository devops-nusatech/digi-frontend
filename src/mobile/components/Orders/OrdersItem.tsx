...
const OrdersItemComponent = props => {
   ...
   const getOrderType = (side: string, type: string) => {
      ...
      return intl.formatMessage({
         id: `page.mobile.orders.header.orderType.${side}.${type}`,
      });
   };

   const currentMarket = (markets.length &&
      markets.find(m => m.id === order.market)) || {
      name: '',
      price_precision: 0,
      amount_precision: 0,
   };
   ...
   const filled = (
      (order.executed_volume / Number(order.origin_volume)) *
      100
   ).toFixed(2);
   const actualPrice =
      order.ord_type === 'market' || order.state === 'done'
         ? order.avg_price
         : order.price;
      ...
      <div
         ...
         <div className="pg-mobile-orders-item__row">
            <div className="pg-mobile-orders-item__row__block">
               <span style={{ color: setTradeColor(order.side).color }}>
                  {orderType}
               </span>
         ...
         <div className="pg-mobile-orders-item__row">
            <div className="pg-mobile-orders-item__row__block">
               ...
               <span className="pg-mobile-orders-item__row__block__value">
                  <Decimal fixed={currentMarket.amount_precision}>
                     {order.origin_volume}
                  </Decimal>
               </span>
            </div>

...
class HistoryComponent extends React.Component<Props> {
      ...
      return (
         <div
            className={`pg-history-elem ${
               list.length ? '' : 'pg-history-elem-empty'
            }`}>
            {fetching && (
   ...
   public renderContent = () => {
      const { core, firstElemIndex, lastElemIndex, page, nextPageExists } =
         this.props;

      return (
         <React.Fragment>
            <History
               headers={this.renderHeaders(core)}
   ...
   private retrieveData = () => {
      ...
      return [...list].map(item => this.renderTableRow(core, item));
   };
   ...
   private renderTableRow = (type, item) => {
      ...
      switch (type) {
         case 'deposits': {
            const { amount, confirmations, created_at, currency, txid, rid } =
               item;
         ...
         case 'trades': {
            ...
            return [
               localeDate(created_at, 'fullDate'),
               <span
                  style={{ color: setTradesType(side).color }}
                  key={id}>
                  {sideText}
               </span>,
               marketName,

...
class OrderBookContainer extends React.Component<Props, State> {
   ...
   public lastPrice() {
      const { currentMarket, isMobileDevice, lastRecentTrade, marketTickers } =
         this.props;
      ...
      if (currentMarket) {
         ...
         const cn = classNames('', {
            'cr-combined-order-book__market-negative':
               priceChangeSign === 'negative',
            'cr-combined-order-book__market-positive':
               priceChangeSign === 'positive',
         });
              ...
              switch (side) {
                 case 'asks':
                    total = isLarge
                       ...
                       : accumulateVolume(array.slice(0).reverse())
                            .slice(0)
                            .reverse();
   ...
   private handleOnSelectAsks = (index: string) => {
      ...
      const asksData = isLarge ? asks : asks.slice(0).reverse();
      const priceToSet =
         asksData[Number(index)] && Number(asksData[Number(index)][0]);
      if (currentPrice !== priceToSet) {
         this.props.setCurrentPrice(priceToSet);
      }
   };
